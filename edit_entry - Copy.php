  ...
  <script>
    ...
    document.querySelectorAll(".cancel-btn").forEach(function (btn) {
      btn.addEventListener("click", function () {
        const container = btn.closest(".land-row");
        if (canRemoveSection(container)) {
          container.remove();
        }
      });
    ...
    <div id="<?= $groupId ?>" style="background:#e4e4e4; padding:15px; border-radius:15px; margin-bottom:15px;">
      ...
      <div id="<?= $groupId ?>_rows">
        <?php foreach ($lands as $land): ?>
          <div style="border:1px solid #ccc; padding:10px; margin-top:10px; background: #fff;" class="land-row">
            ...
            <input type="hidden" name="village_group[]" value="<?= $groupId ?>">
            <button type="button" class="btn btn-danger cancel-btn" style="margin-left: 10px;">Cancel</button>
          </div>
        <?php endforeach; ?>
